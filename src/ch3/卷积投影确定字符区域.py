...
def distributeImg(imgInfoList):
    ...
    for imginfo in imgInfoList:
        ...
        i=i+1
        plt.subplot(3,1,i)
        if(blurName=='原图' or blurName=='切分后的图'):
...
def cutImg(img,startIndex,width):
    #frame[120:360, 160:480]
    ret=img[:,startIndex:startIndex+width]
    return ret
...
startIndex=np.unravel_index(np.argmax(convolve[0]),convolve[0].shape)
startIndex=startIndex[0]
cutimg=cutImg(bit_wise,startIndex,900)
cutimgInfo=[cutimg,0]
nameList=['原图','卷积算法','切分后的图']
imgInfoList=[sorImg,convolve,cutimgInfo]
fullImgInfoList=[]
#将信息添加到数组里面
